  ...
  return (
    <>
      <div
        ...
        className={`fixed inset-0 z-50 bg-black/60 backdrop-blur-sm transition-opacity ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
      />
      <aside
        className={`fixed top-0 right-0 z-50 h-full w-full sm:w-[420px] bg-background shadow-hero transition-transform duration-500 ${isOpen ? "translate-x-0" : "translate-x-full"} flex flex-col`}
      >
          ...
          {items.length === 0 && (
            <div className="text-center py-20 text-muted-foreground">
              ...
              <Link
                ...
                onClick={() => setOpen(false)}
                className="inline-block mt-4 text-sm font-semibold text-primary cm-link"
              >
                Start shopping →
              </Link>
            </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between mt-2">
                  <div className="flex items-center border rounded-md">
                    <button onClick={() => setQty(i.key, i.qty - 1)} className="p-1.5 hover:bg-muted">
        ...
        {items.length > 0 && (
          <div className="border-t p-5 space-y-3">
            ...
            <button className="w-full bg-primary text-primary-foreground py-3 rounded-md font-semibold hover:bg-primary-glow transition-colors">
              Checkout · ₹{(subtotal + (subtotal >= 999 ? 0 : 99)).toLocaleString()}
            </button>
            <Link
              to="/category/apparels"
              onClick={() => setOpen(false)}
